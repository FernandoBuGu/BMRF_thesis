#Script with the functions that are required in the script "step6_c_AUC.R"
...
AUCf<-function(go,k,all,noit_GS=30,PR=0,RMO=0)
{
	...
	print("GOfile")  #How can this be same?
	print(dim(myGO$GOfile))
	print("dimD_m")
	print(dim(myGO$D_m))
...
#" network: a data_frame corresponding to the network file as loaded
	...
	GOfile<-read.table("/home/WUR/bueno002/chi35_1GO/additional/go_complete.tsv" ,header=F)
	network<-read.table("/home/WUR/bueno002/chi35_1GO/big_topless035.tsv" ,header=F,fill=T)
...
#"			(depending on "reduce")
	...
	print("lengthDF")
	print(length(DF))
	domains<-read.table("/home/WUR/bueno002/chi35_1GO/big_ipr_labels_as_humans.tsv" ,header=F)
