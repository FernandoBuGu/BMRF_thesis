#Scrit with the functions that are required in the other scripts within the same folder
...
AUCf<-function(go,k,all,noit_GS=30,PR=0,RMO=0)
{
#computes Area Under the curve (AUC) for a given GO term.

#In:
    #go:int, from 1 to length only-GO-file, index of the GO term of interest in the only-GO-file
    #k: int, from 1 to the number of folds, refers to the fold.
...
#Out:
	...
	library(Matrix);
	myGO<-reduce_myGO(go,reduce,PR,RMO)
	...
	go_number<-go	
	GOfile=myGO$GOfile
	names<-GOfile[as.character(GOfile$go) == as.character(go_number),] # select only the associations of the target GO term
	valid_positive_GO<-names[names$reliab == "valid",]  #select only the "valid" associations


	data_fold<-create_Lsingle2(k,all=all,myGO)#create the folds
	AUCs<-c()
	for(i in 1:length(data_fold$dat)){
        #execute the functions glmnetDalpha, BMRFz and calibrate fom Yiannis code "bmrf_functions.R"
        ...
        #both, the posteriors and the labels have as names the genes.
		posteriors_test<-posteriors[names(posteriors) %in% names(unlist(data_fold$test[i]))] #look within the test set	
		posteriors_test<-posteriors_test[order(names(posteriors_test))] #order so that posteriors and labels match
		labels_test<-unlist(data_fold$test[i])  #look within the test set	
		labels_test<-labels_test[order(names(labels_test))] #order so that posteriors and labels match
		library(AUC)	
		AUCc=try(auc(roc(posteriors_test,as.factor(labels_test))));#contrast the predicted posteriors and the true labels of the test set
...
#In: 
    # k: an integer that refers to the number of folds in the cross validation
    # all: a logical that if true transforms an integer referring to the (index) of the go-term in L_m into a go_number (or "GO ID")
    # myGO: a list with the data frames that are required to run BMRF (A, L_m and D_m) after removing PR % of the rows

#Out:
    #lista: list, contains test, dat and labels
        #test: The genes in the test set
        #dat: GO-file for the GO term of interest
        #labels: The genes in the train set
...
#In:
    #v: vector of 1s or 0s
    #k: int, number of the fold

#Out:
    #list_test: list, each element is the train set of a fold
...
# 		/home/bueno002/humans/run/run_subsets/network_subsets. If false, all network data will be used.
...
# network: a data_frame corresponding to the network file as loaded
	...
	GOfile<-read.table("/home/bueno002/chi35_1GO/additional/go_complete.tsv" ,header=F)
	network<-read.table("/home/bueno002/chi35_1GO/big_topless035.tsv" ,header=F,fill=T)
...
reduce_myGO<-function(go,reduce=F,PR=0,RMO=0){
...
#			(depending on "reduce")
	...
	print("lengthDF")
	print(length(DF))
	domains<-read.table("/home/bueno002/chi35_1GO/big_ipr_labels_as_humans.tsv" ,header=F)
