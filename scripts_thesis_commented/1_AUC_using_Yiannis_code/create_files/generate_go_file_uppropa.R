#Script to generate the GO-file after uppropagating. This is a continuation of the script generate_input_file_for_yeast.R
...
#"large" stands for including non-valid associations, whereas "small" stands for considering only the valid assocations. In the two scenarios, the Go file are uppropagated independetly. 
...
write.table(small_up,file="go_file_small.uppropagated_ready",sep="\t", col.names = F, row.names = F, quote = F)
    ...
    #visualize a density plot of nº proteins per GO term
	tabl<-table(small_up)
	j<-as.matrix(tabl)
	h<-colSums(j)
	z<-sort(as.numeric(h))
...
write.table(up_evid,file="go_file.uppropagated_extended_ready",sep="\t", col.names = F, row.names = F, quote = F) 	#This have 4 columns but allows to continue
up_evid$term<-NULL
up_evid$evid<-NULL
