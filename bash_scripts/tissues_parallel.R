#sc ript to run PU-BMRF analysis with data from all tissues. This R script runs in parallel, so run in machine with enough number of cores
#qsub -q all.q -t 1-1887 tissues_5r.sh
args<-commandArgs(T)
old_start <- Sys.time()
library(Matrix);
